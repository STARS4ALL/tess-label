...
from tesslabel          import TESSW, TESSP, TAS, REF, TEST
from tesslabel.logger   import setLogLevel
from tesslabel.utils    import chop

from tesslabel.photometer.protocol.tessw import TESSUDPProtocol
...
class PhotometerService(Service):
    ...
    def connect(self):
        ...
        if proto == 'serial':
            ...
            self.gotProtocol(protocol)
            self.log.info("listening on UDP endpoint {endpoint}", endpoint=self.options['endpoint'])
    ...
    def buildFactory(self, old_payload, proto):
        if self.options['model'] == TESSW:
            import tesslabel.photometer.protocol.tessw
            factory = tesslabel.photometer.protocol.tessw.TESSProtocolFactory(
                model       = TESSW,
                log         = self.log,
            ...
            import tesslabel.photometer.protocol.tessp
            factory = tesslabel.photometer.protocol.tessp.TESSProtocolFactory(
                model     = TESSP, 
                log       = self.log,
                namespace = self.msgspace,
            )
        else:
            import tesslabel.photometer.protocol.tas
            factory = tesslabel.photometer.protocol.tas.TESSProtocolFactory(
                model     = TAS, 
                log       = self.log,
                namespace = self.msgspace,
            )
